import BlasterController from "./blaster-controller.js";
...
export default class PlayerController {
  ...
  async prepare() {
    ...
    const maxHorizontalSpeed = 9.8 * 1.5; // Maximum horizontal speed
    const maxVerticalSpeed = 9.8 * 1.5; // Max vertical speed due to gravity
    ...
    scene.onBeforePhysicsObservable.add(() => {
      ...
      const hitInfo = this.touchingFloor(scene);
      ...
      if (this.pressedKeys.Space && jumpState === 'idle') {
        ...
        const currentVelocity = ball.physicsAggregate.body.getLinearVelocity();
      ...
      const velocity = ball.physicsAggregate.body.getLinearVelocity();
  ...
  launchPlayer() {
    ...
    const currentVelocity = this.colliderBall.physicsAggregate.body.getLinearVelocity();
  ...
  touchingFloor(scene) {
    ...
    const rayOrigin = this.colliderBall.position; // .getAbsolutePosition(); //scene.activeCamera.position;
    const rayDirection = new BABYLON.Vector3(0, -1, 0); // Downward direction

        // Define the ray with a length of 0.7 units
    const rayLength = this.playerHeight / 2 + 1;
    const downwardRay = new BABYLON.Ray(rayOrigin, rayDirection, rayLength);

        // Check intersection with all meshes in the scene
    const hitInfo = scene.pickWithRay(downwardRay);
